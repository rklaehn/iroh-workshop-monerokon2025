...
async fn accept() -> Result<()> {
    // configure iroh endpoint with discovery (dns and pkarr)
    // print a short ticket
    // otherwise like like echo2
    todo!();
}
...
async fn connect(message: &str, ticket: &str) -> Result<()> {
    // configure iroh endpoint with discovery (dns and pkarr)
    // parse the short ticket and connect
    // otherwise like echo2
    todo!();
}
